// NOTE: the upstream React Bits copy had a webcam face-tracking mode driven
// by face-api.js. It was stripped here: the site never enables it (the bits
// wrapper passes no webcam props), yet the static import welded a ~650KB
// ML stack into the chunk and the model-loading effect fetched CDN weights
// on every mount. Pointer / gyro control is untouched.
...
export const GridScan = ({
  ...
  useEffect(() => {
    ...
    renderer.setPixelRatio(dprForTier());
    ...
    // Skip GPU work (bloom + chromatic aberration multi-pass) while the
    // section is offscreen or the tab is hidden — same gating as the other
    // background components. Idle rAF ticks are negligible.
    let isVisible = true;
    let io = null;
